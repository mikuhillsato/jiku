...
export default function LifePathCalculator() {
  ...
  return (
    <div className="bg-[#1A1714] text-[#F5F1EA] p-8 md:p-12">
      ...
      {result !== null && meaning && (
        <div className="border border-[#3A3530] p-6 animate-fade-in">
          <div className="flex items-baseline gap-4 mb-4">
            <span className="font-display text-7xl text-[#C4926A] font-light leading-none">
              {result}
            </span>
            <div>
              <p className="text-xs text-[#8A7E75] tracking-wider mb-1">Life Path Number</p>
              <p className="text-sm text-[#C4926A] tracking-wider">{meaning.keyword}</p>
            </div>
          </div>
          <p className="text-sm text-[#D6C5B0] leading-loose tracking-wider">{meaning.desc}</p>
        </div>
      )}
    </div>
  );
